...
<!-- modal box end -->
                          					...
                          					<a class='text-primary' data-toggle='tooltip' title='Edit' href='<?php echo site_url("admin/appointment?expert_id=".$mv['expert_id']."&date=".$mv['date'])?>'><i class='fa fa-pencil'></i></a>                          					
                          					<a title="Delete" data-toggle="tooltip"  href="<?php echo site_url('admin/appointment/delete_appoints/'.$mv['expert_id'].'/'.$mv['date'])?>" onclick="return confirm('Are you sure you want to delete records of the selected date?')" class="text-danger"><i class="fa fa-close"></i></a>
                          					</td>
        								   </tr>
        									<?php
        									$sr_no++;
...
function delete_time(time_row_id){
	$.post("<?php echo site_url('admin/appointment/delete_time')?>",{"row_id":time_row_id},function(res){
		console.log(res);
