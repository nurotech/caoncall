...
class Appointment_model extends CI_Model {
    ...
    /**
     * Save appointment in table expert_appointment
     * @param array $passed
     * @param void
     * */
    function _save_appointment($passed){
        $this->db->insert("expert_appointment",$passed);        
        return $this->db->insert_id();
    }
    
    /**
     * Save appintment time
     * */
    function _save_appoint_time($array){
        $this->db->insert("appoint_time",$array);
        return $this->db->insert_id();
    }   
    ...
    function get_service_service_id($service_id){        
        return $this->db->order_by("name","ASC")->get_where("services",array("id"=>$service_id))->result_array();        
    }
    
        //get service name by service_id
    function get_service_id_by_service_name($service_name){         
        $list= $this->db->get_where("services",array("name"=>$service_name))->result_array();
       return $list['0']['id'];       
    } 
    
        //   getting data
    function ApptByExpertidDateService($expert_id,$list,$service_id_service_name){
        //return $this->db->where(['expert_id'=>$expert_id,'date'=>$list,'service_id'=>$service_id_service_name])->group_by(array("date","expert_id"))->get('expert_appointment')->result_array();
        return $this->db->where(['expert_id'=>$expert_id,'date'=>$list,'service_id'=>$service_id_service_name])->get('expert_appointment')->result_array();
    ...
    function saveAppointment($data){      
     ...
     function update_category_status($cat_id,$status){
        ...
        //list of categories
    function categoryList($filter_cid){
        if($filter_cid){
    ...
    /*  function ApptByExpertidDate($expert_id,$list,$service_name){
          return $this->db->where(['expert_id'=>$expert_id,'date'=>$list,'service_id'=>$service_id])->group_by(array("date","expert_id"))->get('expert_appointment')->result_array();
      ...
      public function selectAppt(){
        ...
        $rows=$this->db->group_by(array("date","service_id","expert_id"))->get_where('expert_appointment',array("expert_id"=>$expert_id))->result_array();
       return $rows;
      }
